...
class WMInteger:
    ...
    def line_select(self):
        ...
        for i in range(self.first_module, self.last_module + 1):
            dict1 = self.features()
            dict1[":IOInt"] = "WM{}_LineSelect".format(i)
            dict1["ItemName"] = "WM{}_LineSelect.CSW".format(i)
    ...
    def control_wm_inactive(self):
        dict_data = self.line_select()
        for i in range(self.first_module, self.last_module + 1):
            dict1 = self.features()
            dict1[":IOInt"] = "WM{}_Control_WM_Inactive".format(i)
            dict1["ItemName"] = "WM{}_Control.TMR_Inactive.PRE".format(i)
    ...
    def create_csv(self):
        csv_file = "csv-files/integer/wm_integer.csv"
        dict_data = self.control_wm_inactive()
